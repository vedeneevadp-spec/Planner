...
Удаление встроенного голосового ввода имеет отдельный
[план выпуска и отката](voice-removal/release-plan.md): старый backend
несовместим с удалением столбцов в миграции 000106. Обычный автоматический
откат на старую версию после этой миграции не подходит.
...
2. API отвечает на `/api/health`, а `/api/ready` подтверждает готовность зависимостей
...
Если релиз затрагивает backend schema, `npm run deploy:prod` сам снимает backup,
запускает migration runner и проверяет RLS/security-инварианты. Ручной запуск
...
VPS, чтобы использовать тот же сетевой контур и `/etc/planner/planner.env`:
...
- до релиза новые пункты собираются в `Unreleased`
- перед rollout раздел переносится в `vX.Y.Z - YYYY-MM-DD`
- в разделе указываются каналы: `Web`, `PWA`, `iOS`, `Android`
- текст должен описывать пользовательский эффект, а не внутреннюю реализацию
- store release notes копируются из того же раздела с учетом лимитов площадки
...
1. `https://chaotika.ru`
2. `https://chaotika.ru/api/ready`
3. логин
4. основные пользовательские экраны
5. PWA install/open flow в production браузере
6. release notes соответствуют фактически выкаченным каналам
...
2. проверить `GET /api/health` и `GET /api/ready`
...
6. опубликовать store release notes из `docs/release-notes.md`
...
2. если менялись SQL/RLS, прогнать `npm run test:api:postgres`
3. обновить `docs/release-notes.md`, если изменение видно пользователю или
   поддержке
4. `npm run deploy:prod`
5. smoke production web и API

### Web/PWA релиз

1. `npm run ci`
2. `npm run build && npm run preview`
3. обновить `docs/release-notes.md`
4. `npm run deploy:prod`
5. проверить production web и PWA
...
5. обновить `docs/release-notes.md`
6. `npm run deploy:prod`
7. собрать и загрузить store builds
8. проверить production web, PWA и release build на устройстве
