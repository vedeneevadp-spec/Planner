...
`genericJsonObjectSchema()` в bootstrap OpenAPI снижен с 31 до 15 использований:
chaos inbox, daily plan и push DTO получили явные component schemas из
contracts. `npm run typecheck:api` и `npm run openapi:check` проходят после
правки. Оставшиеся generic schemas - Alice OAuth/webhook, life spheres/profile
и несколько voice/push-support response wrappers; Alice нужно закрывать
отдельным fixture-backed contract, потому что это внешний webhook/OAuth
протокол.
