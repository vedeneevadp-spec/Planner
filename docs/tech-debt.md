...
Дата анализа: 2026-05-21. Обновлено: 2026-09-06.
...
## Закрыто в коде 2026-09-06: четыре P1 из свежего аудита

1. TD-01: обновлены Fastify и уязвимые транзитивные зависимости; proxy trust
   задаётся IP/CIDR. Старое значение `API_TRUST_PROXY_HOPS=1` совместимо через
   доверие только loopback. Оба npm audit gate проходят без новых исключений.
2. TD-02: преобразование локального времени использует ограниченный кэш Intl
   formatter и проверку смещений вместо перебора каждой минуты. Self-care
   использует общий результат без дополнительного SQL-запроса на каждую дату.
3. TD-03: страницы календаря/истории включены в область инвалидирования задач;
   оптимистические изменения, ответы сервера и откаты синхронизируют кэши,
   а отображение выбирает актуальную версию записи.
4. TD-04: команды находят versioned record и в загруженных страницах; задачи
   вне ограниченного snapshot доступны для редактирования, статуса и удаления,
   в том числе через офлайн-очередь.

Подробности и локальная проверка: [отчёт об исправлениях](audits/2026-09-06-p1-fixes.md).
Исходный аудит и оставшиеся P2/P3 сохранены в
[отдельном отчёте](audits/2026-09-06-tech-debt.md). Production-выпуск не выполнялся.
...
- `apps/web/src/pages/self-care/ui/SelfCarePage.helpers.ts` - 1881 строка
- `apps/api/src/bootstrap/openapi-contract-schemas.ts` - 1829 строк
...
Актуальный смещенный риск 2026-07-02: самые крупные hotspots теперь не только
auth/session repositories и voice. Главная стоимость изменений остается в
self-care UI/backend, AI context aggregation, ручном OpenAPI layer и coverage
gates для новых горячих зон:
...
Статус 2026-07-02: create/edit dialog shell вынесен в
`SelfCarePage.dialogs.tsx`, а общий marker состояния icon picker - в
`SelfCarePage.icon-picker-state.ts`. Формы create/edit пока остаются
экспортированными из `SelfCarePage.components.tsx`, чтобы следующий перенос был
...
`npm run -w apps/api test -- src/modules/self-care/self-care.shared.test.ts`,
`node --import tsx --test src/modules/self-care/self-care.repository.postgres-spec.ts`,
`npm run coverage:api` и sequential Postgres coverage gates проходят. Новый
остаток для backend self-care - не поведенческий snapshot loader, а физическая
граница модулей: read-model loaders стоит вынести из repository class, а
projection algorithms - постепенно отделять от migration/template helpers в
`self-care.shared.ts`.
...
Статус 2026-07-02: inventory обновлен под текущие размеры файлов.
`genericJsonObjectSchema()` в bootstrap OpenAPI снижен с 31 до 14 component
вызовов, плюс 1 helper definition. Chaos inbox, daily plan и push DTO получили
явные component schemas из contracts. `npm run typecheck:api` и
`npm run openapi:check` проходят после правки. Оставшиеся generic schemas -
Alice OAuth/webhook, life spheres/profile, voice `intent`/`stt` wrappers и
`WeeklySphereStatsRecordResponse`; Alice нужно закрывать отдельным
fixture-backed contract, потому что это внешний webhook/OAuth протокол.
...
### Закрыто 2026-07-02: Codex release instruction синхронизирована

`docs/codex-release-instruction.md` теперь требует классифицировать тип релиза,
обновлять release notes, выбирать gates по типу изменений, фиксировать native
version bump для публичного APK/store build и явно предупреждать, что APK без
version bump является технической сборкой, а не полноценным store release.
Также зафиксировано, что `npm run deploy:prod` требует clean pushed branch и
сам запускает `npm run ci`, если не указан `--skip-checks`.

### Нужен регулярный dependency batch без смешивания с фичами

Проблема: `npm audit --omit=dev` и `npm run audit:dev-tooling` чистые, но
`npm outdated` на 2026-07-02 показывает заметный patch/minor drift в runtime и
tooling зависимостях. В списке есть React `19.2.5 -> 19.2.7`,
React Router `7.16.0 -> 7.18.1`, Capacitor `8.3.3 -> 8.4.1`,
`@capacitor/push-notifications` `8.0.4 -> 8.1.1`, Fastify `5.8.5 -> 5.9.0`,
`pg` `8.20.0 -> 8.22.0`, Zod `4.3.6 -> 4.4.3`, Vite `8.0.16 -> 8.1.3`,
Vitest `4.1.4 -> 4.1.9`, Playwright `1.59.1 -> 1.61.1` и TypeScript
`6.0.2 -> 6.0.3`. Major drift тоже виден для ESLint/jsdom/lint-staged, но его
лучше не смешивать с обычным patch/minor batch.

Что сделать:

- отдельным PR обновлять patch/minor batches по группам: frontend runtime,
  backend runtime, mobile native, test/tooling
- для Capacitor batch запускать `npm run mobile:ci-check`,
  `npm run mobile:android:ci` и ручную проверку native auth/push flows
- для React/Router/TanStack batch запускать `npm run test:web:run`,
  `npm run build:budget` и mobile installed-auth smoke
- major tooling drift держать отдельным пунктом с dry-run CI, а не включать в
  поддерживающие dependency updates
...
### Postgres coverage jobs требуют изоляции при параллельном запуске

Проблема: `npm run coverage:api:postgres-contracts` и
`npm run coverage:api:postgres-pooler-contracts` проходят sequential, но при
одновременном локальном запуске на общей Postgres test DB могут загрязнять
global icon set state. Свежий срез 2026-07-02: параллельный запуск дал
false-negative в `PostgresEmojiSetRepository contract`, затем оба job прошли
по отдельности: postgres-contracts `22/22`, pooler-contracts `7/7`.

Что сделать:

- не запускать эти два job параллельно на одной локальной DB
- если нужен параллельный CI/multi-agent прогон, выделять отдельную DB/schema
  на job или добавить namespace/cleanup для global icon set fixtures
- оставить `npm run coverage:api:postgres` как sequential gate, пока изоляция
  не вынесена в инфраструктуру тестов
...
Статус 2026-07-02: текущие budget-факты обновлены после self-care asset cleanup
и route split. `npm run build:budget` проходит с entry JS `244.6 KB`, initial
JS `816.6 KB`, self-care route JS `89.1 KB`, self-care route CSS `34.3 KB`,
calendar route JS `33.0 KB`, calendar route CSS `23.8 KB`, VoiceAssistant JS
`82.6 KB`, VoiceAssistant CSS `7.5 KB`, planner-contracts JS `188.5 KB`,
lottie canvas JS `199.4 KB`, public self-care assets `558.8 KB` и public icon
assets `608.1 KB`. Android budgets проходят с debug APK `153.1 MB`, release APK
`43.7 MB` и release AAB `71.4 MB`.
...
Статус 2026-07-02: свежий `npm run coverage:web` проходит, общий web lines
`69.29%`. Текущие hotspots выше старых минимумов, но guard пока не подтянут к
факту: `useSelfCare.ts` `33.67%`, `self-care-api.ts` `55.55%`,
`native-push-notifications.ts` `68.65%`, `TimeZoneChangeBanner.tsx` `83.78%`,
`useWorkspaceParticipants.ts` `37.61%`. Следующий coverage step - поднять
минимумы для уже покрытых файлов и добавить явные targets для
`useVoiceActionFlow.ts` (`46.03%` lines), `mcp-haotika.server.ts` (`49.18%`
API lines) и self-care Postgres read-model loaders, которые сейчас проверяются
spec-тестами, но не входят в API memory coverage report.
