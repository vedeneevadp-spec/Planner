...
5. Довести модель действий до v1.
   Статус: реализован web action layer `PlannerActionExecutor` с контрактом
   `VoiceActionPreview` / `VoiceActionResult`.
   ...
   policy и offline behavior. Подробности зафиксированы в
...
13. Подключить production LLM fallback provider для `PlannerIntentParser`.
...
14. Провести закрытое тестирование.
    ...
    подтверждения. Отдельно сравнить режимы rule-parser only и rule-parser +
    LLM fallback, если пункт 13 включен для `owner/test`. По результатам
    обновить provider settings, parser rules, LLM fallback policy и тексты
    карточки.

15. Подготовить production release gate.
    ...
    а offline/manual voice fallback корректно работает без сети.

16. После релиза улучшать по данным.
