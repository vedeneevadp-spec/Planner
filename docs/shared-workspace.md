...
| -------------------------------------- | ----------- | ----------------------------------------- | ------------------------------------------------------ |
| Глобальный уровень приложения          | `appRole`   | `owner`, `admin`, `test`, `user`, `guest` | доступ к admin-функциям и feature-gates приложения     |
...
- `test` - обычный пользователь с доступом к тестовым feature-gates, сейчас к
  голосовому вводу
...
Активное приглашение не создает membership автоматически. Пользователь видит
входящее приглашение после входа в аккаунт с совпадающим email и вручную
...
При отклонении invitation получает `declined_at`/`declined_by` и остается в
списке инвайтов workspace со статусом `declined`. Повторное приглашение того же
email сбрасывает отклонение и снова переводит invitation в `pending`.

### Выход из shared workspace

Участник shared workspace может выйти через отдельный leave flow. Owner не может
выйти из собственного workspace: для него доступно удаление workspace или будущий
...
| Использовать голосовой ввод v1                  | `appRole = owner` или `test`  |

На текущий момент `workspace settings` содержат только
`taskCompletionConfettiEnabled`.

`appRole = test` не дает admin-права и не меняет права workspace. Это роль
уровня обычного пользователя плюс доступ к тестовым функциям.
