...
-- Exact retries of a committed rotation may outlive the short replay-recovery
-- window. The API derives the same replacement id and hash from a durable
-- client rotation request id, so this branch can return the original outcome
-- without weakening detection of a different replay.
...
begin
  ...
  if current_token.rotated_at is not null then
    ...
    -- A retry carrying the exact deterministic replacement is the same
    -- operation, even after the legacy five-minute recovery window. It is
    -- safe only while that replacement remains the active family token.
    ...
    -- A different same-device replay keeps the narrow legacy recovery window.
    if current_token.rotated_at + recovery_window < resolved_now then
      update auth_refresh_tokens as token
      set revoked_at = resolved_now
      where token.session_id = current_token.session_id
        and token.revoked_at is null;
