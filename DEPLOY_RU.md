...
Для удаления встроенного голосового ввода требуется двухэтапный выпуск:
сначала совместимый API, затем удаление столбцов. Следуйте
[плану перехода](docs/voice-removal/release-plan.md), прежде чем запускать
обычный deploy: миграции выполняются до переключения приложения.
...
HAOTIKA_OAUTH_ALLOWED_REDIRECT_URIS=https://chatgpt.com/connector/oauth/<callback_id>
