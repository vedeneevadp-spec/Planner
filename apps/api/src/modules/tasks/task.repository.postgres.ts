...
export class PostgresTaskRepository implements TaskRepository {
  ...
  async listByWorkspace(
    ...
    const taskRows = await query.orderBy('created_at', 'asc').execute()
    const timeBlockMap = await this.loadPrimaryTimeBlocks(workspaceId, taskRows)
  ...
  async updateStatus(
    ...
    if (!updatedTask) {
      const currentTask = await this.loadCurrentTask(command)
    ...
    await this.writeTaskMutationArtifacts(this.db, {
      actorUserId: command.context.actorUserId,
  ...
  async updateSchedule(
    ...
    const timeBlock = await this.insertPrimaryTimeBlock(this.db, {
      actorUserId: command.context.actorUserId,
      endsAt,
      startsAt,
      taskId: command.taskId,
  ...
  private async loadPrimaryTimeBlocks(
    ...
    const timeBlocks = await this.db
  ...
  private loadCurrentTask(command: {
    context: {
      workspaceId: string
    }
    taskId: string
