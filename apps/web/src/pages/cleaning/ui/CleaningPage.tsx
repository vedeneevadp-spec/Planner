...
type FocusMode = 'all' | 'quick' | 'minimum' | 'regular' | 'deep'
...
const PRIORITY_LABELS: Record<CleaningPriority, string> = {
  high: 'важно',
  low: 'низкий',
  normal: 'обычно',
}

const DEPTH_LABELS: Record<CleaningDepth, string> = {
  deep: 'генеральная',
  minimum: 'минимум',
  regular: 'обычная',
}

const ENERGY_LABELS: Record<CleaningEnergy, string> = {
  high: 'много сил',
  low: 'мало сил',
  normal: 'нормально',
}

const ASSIGNEE_LABELS: Record<CleaningAssignee, string> = {
  anyone: 'любой',
  child: 'ребёнок',
  partner: 'партнёр',
  self: 'я',
...
const FREQUENCY_LABELS: Record<CleaningFrequencyType, string> = {
  custom: 'раз в N дней',
...
const DEFAULT_CLEANING_TEMPLATES: ZoneTemplate[] = [
  {
    dayOfWeek: 1,
...
function filterItemsByFocusMode(
  items: CleaningTaskWithState[],
  mode: FocusMode,
...
function getHeroHint(
  today: NonNullable<ReturnType<typeof useCleaningToday>['data']>,
) {
...
function getFocusModeAriaLabel(mode: FocusMode): string {
  if (mode === 'all') {
...
function createActionInput(date: string) {
  return {
    date,
...
function getWeekdayShortLabel(value: number): string {
  return WEEKDAYS.find((day) => day.value === value)?.shortLabel ?? 'Д'
}

function getIsoWeekdayFromDate(date = new Date()): number {
  const day = date.getDay()

  return day === 0 ? 7 : day
}

function formatFrequency(task: CleaningTaskRecord): string {
...
function formatPostponeCount(count: number): string {
  if (count === 1) {
    return '1 раз'
  }
