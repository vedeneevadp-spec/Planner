...
export function useSessionAuthController(): SessionAuthState {
  ...
  const keepDeviceSession = useCallback(
    (
      ...
      recordClientEvent(
        ...
        {
          category: getDeviceSessionKeepCategory(command.reason),
          errorKind: getAuthErrorKind(command.error),
  ...
  const recoverSession =
    useCallback(async (): Promise<SessionRecoveryResult> => {
      ...
      const recovery = (async () => {
        ...
        recordAuthDecisionEvent('auth_recovery_decision', recoveryCommand, {
          hasBlockedRefreshToken: Boolean(blockedNativeRefreshTokenRef.current),
          hasCurrentRefreshToken: Boolean(snapshot.refreshToken),
          ...
          case 'request_refresh':
            try {
              ...
              recordClientEvent(
                'auth_refresh_failed',
                {
                  category: getRefreshFailureCategory(error),
              ...
              switch (refreshFailureCommand.type) {
                ...
                case 'read_latest_native_session': {
                  const latestStoredSession =
                    await readStoredAuthSession().catch((storageError) => {
                      ...
                      recordClientEvent(
                        'auth_storage_failed',
                        {
                          errorKind: getAuthErrorKind(storageError),
                          fallback: 'none',
                          nativeRuntime: isNativeSessionRuntime,
                          operation: 'read_latest_after_refresh_failure',
                          storage: 'native',
                        },
                        { level: 'warn' },
                      )
                  ...
                  recordAuthDecisionEvent(
                    'auth_refresh_storage_decision',
                    storageCommand,
                    {
                      category:
                        getRefreshStorageDecisionCategory(storageCommand),
                      errorKind: getAuthErrorKind(refreshFailureCommand.error),
                      hasLatestStoredRefreshToken: Boolean(
                        latestStoredSession?.refreshToken,
                      ),
                      hasLatestStoredSession: Boolean(latestStoredSession),
                      hasStoredSession: Boolean(
                        refreshFailureCommand.storedSession,
                      ),
                      nativeRuntime: isNativeSessionRuntime,
                      status: getAuthErrorStatus(refreshFailureCommand.error),
                    },
                  )
  ...
  const restoreSession =
    useCallback(async (): Promise<SessionRecoveryResult> => {
      ...
      recordAuthDecisionEvent('auth_restore_decision', restoreCommand, {
        currentHasAccessToken: Boolean(snapshot.sessionAccessToken),
        hasStoredAccessToken: Boolean(storedSession?.accessToken),
        hasStoredRefreshToken: Boolean(storedSession?.refreshToken),
        hasStoredSession: Boolean(storedSession),
...
function recordAuthDecisionEvent(
...
function getDeviceSessionKeepCategory(
...
function getRefreshFailureCategory(error: unknown): string {
...
function getRefreshStorageDecisionCategory(
  command: SessionAuthMachineCommand,
): string {
  switch (command.type) {
    case 'restore_latest_stored_session':
