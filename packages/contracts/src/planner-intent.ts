...
const AGENDA_DATE_QUERY_PATTERN =
  '(?:сегодня|завтра|послезавтра|понедельник(?:е|а)?|вторник(?:е|а)?|сред[ауеы]|четверг(?:е|а)?|пятниц[ауеы]|суббот[ауеы]|воскресень[еяю]|выходные|(?:следующ(?:ую|ей)\\s+)?недел[юе])'
const AGENDA_PATTERN = new RegExp(
  `(?:^|\\s)(?:что\\s+у\\s+меня|что\\s+на\\s+${AGENDA_DATE_QUERY_PATTERN}|какие\\s+(?:задачи|дела|планы)|какие\\s+у\\s+меня(?:\\s+${AGENDA_DATE_QUERY_PATTERN})?\\s+(?:задачи|дела|планы)|что\\s+запланировано|покажи\\s+(?:план|задачи|расписание|дела)|(?:план|дела)\\s+на)(?=\\s|$)`,
  'iu',
)
