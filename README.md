...
- последние snapshots задач, сфер и templates, а также очередь
  write-операций хранятся в IndexedDB через Dexie
- offline queue replay-ится через HTTP API; конфликты версий
  `task_version_conflict` и `life_sphere_version_conflict` остаются в
...
Текущие экраны: `/today`, `/calendar`, `/shopping`, `/cleaning`,
`/cleaning/settings`, `/spheres`, `/spheres/:sphereId`, `/habits`, `/more`,
`/profile`, `/admin`. Старый `/timeline` оставлен как redirect на `/calendar`
с дневным режимом.
...
- для signed APK под RuStore сначала создайте `android/keystore.properties` по
  ...
  `npm run mobile:release:rustore -- --api-url=https://chaotika.ru --version=1.1.4 --build=12`
- если RuStore Console принимает AAB для приложения, предпочтительно собирать
  AAB: он меньше для пользователя, потому что стор генерирует APK под устройство:
...
- `npm run db:migrations:check` проверяет формат имен, монотонность sequence и
  новые дубли номеров миграций; исторический applied-дубль зафиксирован в
